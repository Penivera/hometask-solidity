...
contract EthexLoto is Ownable {
    ...
    function placeBet(bytes22 params) external payable {
        ...
        markedCount = getMarkedCount(bet);
        (jackpotFee, houseEdgeFee, betAmount, houseEdgePercent) = calculateBetAmounts(msg.value, markedCount);
        
        (coefficient, markedCount, holdAmount) = getHold(betAmount, bet);
        
        require(msg.value * (100 - JACKPOT_PERCENT - houseEdgePercent) * (coefficient * 8 - 15 * markedCount) <= 9000 ether * markedCount);
        
        require(
            msg.value * (800 * coefficient - (JACKPOT_PERCENT + houseEdgePercent) * (coefficient * 8 + 15 * markedCount)) <= 1500 * markedCount * (address(this).balance - holdBalance));
    ...
    function calculateBetAmounts(uint256 amount, uint8 markedCount) internal pure returns (uint256 jackpotFee, uint256 houseEdgeFee, uint256 betAmount, uint256 houseEdgePercent) {
        houseEdgePercent = getHouseEdgePercent(markedCount);
        jackpotFee = amount * JACKPOT_PERCENT * PRECISION / 100 / PRECISION;
        houseEdgeFee = amount * houseEdgePercent * PRECISION / 100 / PRECISION;
        betAmount = amount - jackpotFee - houseEdgeFee;
    }

    function getHouseEdgePercent(uint8 markedCount) internal pure returns (uint256) {
    ...
    function getMarkedCount(bytes6 bet) internal pure returns (uint8 markedCount) {
